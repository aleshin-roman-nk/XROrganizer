...
namespace TaskBank.ViewModules
{
	public class TaskCollectionView
	{
		...
		public RmTask CurrentTask => bs.Current as RmTask;
		RmTask _previousTask { get; set; }

		public event EventHandler<RmTask> SaveTaskNeeded;
		public event EventHandler<IEnumerable<RmTask>> MoveTaskCollection;
		...
		private void _grid_CellFormatting(object sender, DataGridViewCellFormattingEventArgs e)
		{
			...
			row.Cells["Imp"].Style.BackColor = imp_to_color((row.DataBoundItem as RmTask).Imp);
			row.Cells["Imp"].Style.SelectionBackColor = imp_to_color((row.DataBoundItem as RmTask).Imp);
		...
		private void _commitAndSave(RmTask task)
		{
		...
		private void _put(RmTask task)
		{
			_taskBody.Text = task.Text;
			savingObserver.Saved = true;
		}

		private IEnumerable<RmTask> _getSelectedTasks()
		{
			//List<RmTask> res = new List<RmTask>();

			return _grid.SelectedRows.Cast<DataGridViewRow>().Select(x=>x.DataBoundItem as RmTask);
		...
		public void DisplayTaskCollection(IEnumerable<RmTask> tcollection)
		{
			savingObserver.Saved = true;
