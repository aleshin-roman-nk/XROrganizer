...
namespace TaskBank
{
	public class MainPresenter
	{
		...
		ISingleEntityView<RmTask> _task_view;

		public MainPresenter(IMainView mv, IRmTaskRepository repo, ISingleEntityView<RmTask> task_view)
		{
			_mainView = mv;
			_repo = repo;
		...
		private void _mainView_SaveTaskCommand(object sender, RmTask e)
		{
			_repo.Save(e);
			_mainView.DisplayTaskCollection(_repo.GetAll());
		}

		private void _mainView_DeleteNoteCommand(object sender, RmTask e)
		{
			if (e == null) return;
		...
		{
			var res = _task_view.Go(new RmTask { Text = $"{getTimeStringOfNow(e)}{Environment.NewLine}" });
